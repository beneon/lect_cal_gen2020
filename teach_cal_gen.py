...
doc = Document("教学日历模板.docx")



xlFile = pd.ExcelFile('郑骏明负责课程.xlsx')
dtypes = {
    '节数':str,
    '节次':str,
    '周数':int,
...
df_kouchuang_spss = xlFile.parse('17口创',dtype=dtypes)
df_yaoxue_spss = xlFile.parse('18药学',dtype=dtypes)
...
class CourseCalGenerator:
    ...
    with open('2019_2020_weeks.yaml') as data_file:
        weeks_data = full_load(data_file)
