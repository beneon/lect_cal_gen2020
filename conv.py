...
xl_df_col = [
    {'df_name':e, 'df':xlFile.parse(e, skiprows=2, header=None, names=['wday','etc'])} for e in xl_df_col
]
...
class Cleaner:
    weeks_yaml_path = '2019_2020_weeks.yaml'
    # 关键设置，使用前必须要调整，指向xxx_weeks.yaml文件
    alert_logger = alert_logger
    # 对外部代码的alert_logger有依赖，迁移的时候请注意
    with open(weeks_yaml_path) as weeks_yaml:
        weeks_yaml_data = full_load(weeks_yaml)
    ...
    ([^\s]*?\s|[^\s]*?$) #4. 教室
    """,re.VERBOSE)
    # 上面的内容以外，剩下的内容往往编码就比较混乱了，但是节次是应该提取出来的一个信息
...
with open(f'log_{date.today()}.txt','w') as logger_file:
